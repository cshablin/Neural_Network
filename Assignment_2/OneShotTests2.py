...
def load_set(file_name: str, data_dir: str):  # -> Tuple[Dict[int, np.ndarray], np.ndarray]:
    with open(file_name) as pairsFile:
        ...
        for index in range(1, pairs_count + 1):
            ...
            paired_labels.append([1])  # 1 = same person
        # different pairs
        unpaired_images = []
        unpaired_labels = []
        for index in range(1 + pairs_count, 2 * pairs_count + 1):
            ...
            img1 = io.imread(os.path.join(dir1_path, f"{dir1_name}_{img1_name.zfill(4)}.jpg")) # / 255 - divide by 255 only batch, memory saving
            img2 = io.imread(os.path.join(dir2_path, f"{dir2_name}_{img2_name.zfill(4)}.jpg")) # / 255
            unpaired_images.append([img1, img2])
            unpaired_labels.append([0])  # 0 = different person
    return np.array(paired_images), np.array(paired_labels), np.array(unpaired_images), np.array(unpaired_labels)
...
class LoadDataTests(unittest.TestCase):
    ...
    def test_split_train_set(self):
        ...
        train_paired_images, train_paired_labels, train_unpaired_images, train_unpaired_labels = load_set(file_name,
                                                                                                          self.data_folder)
        train_paired_images, train_paired_labels, validation_paired_images, validation_paired_labels = split_set(train_paired_images, train_paired_labels, ratio)
